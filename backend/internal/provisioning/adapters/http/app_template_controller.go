...
type AppTemplateController struct {
	svc *provisionapp.AppTemplateService
}

// NewAppTemplateController 创建应用商店模板控制器实例。
func NewAppTemplateController(svc *provisionapp.AppTemplateService) *AppTemplateController {
...
func writeProvisioningError(c *gin.Context, err error) {
	...
	case errors.Is(err, provisionapp.ErrInvalidParams):
		resp.Fail(c, 4000, provisioningErrorMessage(err, "参数错误"))
	case errors.Is(err, provisionapp.ErrNotFound):
		resp.Fail(c, 4040, provisioningErrorMessage(err, "未找到资源"))
	case errors.Is(err, provisionapp.ErrConflict):
		resp.Fail(c, 4090, provisioningErrorMessage(err, "资源冲突"))
	default:
		resp.Fail(c, 5000, provisioningErrorMessage(err, "内部错误"))
	}
}

func provisioningErrorMessage(err error, fallback string) string {
	var applicationError *provisionapp.Error
	if errors.As(err, &applicationError) && applicationError != nil && applicationError.UserMessage() != "" {
		return applicationError.UserMessage()
	}
	return fallback
}
