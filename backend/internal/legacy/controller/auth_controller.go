...
func buildLoginFailureData(reason string, status *service.LoginAttemptStatus, canResetPassword bool, suggestions ...string) loginFailureData {
...
func (ac *AuthController) Login(c *gin.Context) {
	...
	if ac.loginAttemptSvc != nil {
		...
		if lockStatus.Locked {
			...
			ac.failLogin(c, loginCodeAccountLocked, service.FormatLockMessage(lockStatus.LockRemainingSeconds), buildLoginFailureData(
				"account_locked",
				&lockStatus,
				true,
				"请等待锁定结束后再试",
	...
	user, err := ac.rbacSvc.Authenticate(c.Request.Context(), username, password)
	if err != nil {
		var attemptStatus *service.LoginAttemptStatus
		if ac.loginAttemptSvc != nil && (err == service.ErrAuthPasswordIncorrect || err == service.ErrAuthUserNotFound) {
			...
			if status.Locked {
				ac.recordAuthAudit(c, 0, username, "login", loginCodeAccountLocked, "账号已锁定")
				ac.failLogin(c, loginCodeAccountLocked, service.FormatLockMessage(status.LockRemainingSeconds), buildLoginFailureData(
					"account_locked",
					&status,
					true,
					"请等待锁定结束后再试",
