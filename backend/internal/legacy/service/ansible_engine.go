...
func (s *DeployService) ansiblePipeline(ctx context.Context, planID uint64, taskID int64) {
	...
	task.Steps = make([]platformapp.TaskStep, len(ansibleSteps))
	...
	for i, step := range ansibleSteps {
		...
		task.Steps[i] = platformapp.TaskStep{
			Key:        step.Key,
			Title:      step.Title,
			Status:     status,
			StartedAt:  nil,
	...
	for i := range task.Steps {
		if task.Steps[i].Status != platformapp.StepSuccess {
			task.Steps[i].Status = platformapp.StepRunning
			task.Steps[i].StartedAt = &now
			break
		}
	}
	...
	if err != nil {
		// 标记失败步骤
		for i := range task.Steps {
			if task.Steps[i].Status == platformapp.StepRunning {
				task.Steps[i].Status = platformapp.StepFailed
				errMsg := err.Error()
	...
	for i := range task.Steps {
		if task.Steps[i].Status != platformapp.StepSuccess {
			task.Steps[i].Status = platformapp.StepSuccess
		}
...
func activeDeployStepKey(task *platformapp.Task) string {
	if task == nil {
		return ""
	}
...
func (s *DeployService) registerClusterAfterDeploy(ctx context.Context, plan model.DeployPlan, kubeconfig string) (uint64, error) {
	...
	clusterID, err := s.clusterRegistry.Import(ctx, plan.ClusterName, kubeconfig, "")
	if err != nil {
...
func (s *DeployService) markTaskFailed(task *platformapp.Task, msg string) {
...
func finishUnresolvedSubSteps(step *platformapp.TaskStep, finishedAt time.Time) {
	...
	for i := range step.SubSteps {
		if step.SubSteps[i].Status != platformapp.StepRunning {
			continue
		}
		step.SubSteps[i].FinishedAt = &finishedAt
		if step.Status == platformapp.StepFailed {
			step.SubSteps[i].Status = platformapp.StepFailed
		} else {
			step.SubSteps[i].Status = platformapp.StepSuccess
		}
	}
}

// markTaskCanceled 标记任务取消
func (s *DeployService) markTaskCanceled(task *platformapp.Task) {
