...
func (s *MonitorIncidentService) upsertAlertmanagerIncident(ctx context.Context, alert AlertmanagerAlert) (uint64, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if findErr == nil {
			...
			incidentID = old.ID
			return appendTimelineWithDB(ctx, tx, old.ID, "alert_update", statusTitle(status), row.Summary, 0, "", nil)
		}
		...
		incidentID = row.ID
		return appendTimelineWithDB(ctx, tx, row.ID, "alert_received", "接收 Alertmanager 告警", row.Summary, 0, "", nil)
	})
...
func (s *MonitorIncidentService) linkAI(ctx context.Context, id uint64, field string, value uint64, title string, userID uint64, username string) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		result := tx.Model(&model.MonitorIncident{}).Where("id = ?", id).Updates(map[string]any{field: value, "version": gorm.Expr("version + 1")})
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
		...
		return appendTimelineWithDB(ctx, tx, id, "ai_link", title, fmt.Sprintf("ID: %d", value), userID, username, model.JSONMap{"id": value})
	})
}
func (s *MonitorIncidentService) appendTimeline(ctx context.Context, incidentID uint64, kind, title, detail string, userID uint64, username string, meta model.JSONMap) error {
	return appendTimelineWithDB(ctx, s.db, incidentID, kind, title, detail, userID, username, meta)
}
func appendTimelineWithDB(ctx context.Context, db *gorm.DB, incidentID uint64, kind, title, detail string, userID uint64, username string, meta model.JSONMap) error {
	return db.WithContext(ctx).Create(&model.MonitorIncidentTimeline{IncidentID: incidentID, Type: kind, Title: title, Detail: detail, MetaJSON: meta, OperatorID: userID, Operator: username}).Error
}
