...
func (s *AIRouteSettingsService) Update(ctx context.Context, req UpdateAIRouteSettingsRequest) (AIRouteSettingsItem, error) {
	...
	if err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&model.AIRouteSetting{}).Where("id = ?", row.ID).Updates(updates).Error
	}); err != nil {
		return AIRouteSettingsItem{}, err
	}
...
func (s *AIRouteSettingsService) ensureRow(ctx context.Context) (model.AIRouteSetting, error) {
	var row model.AIRouteSetting
	if err := s.db.WithContext(ctx).First(&row, 1).Error; err == nil {
		return row, nil
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
		return model.AIRouteSetting{}, err
	}

	row = model.AIRouteSetting{
		ID:              1,
...
func buildAIRouteSettingsItem(row model.AIRouteSetting) AIRouteSettingsItem {
