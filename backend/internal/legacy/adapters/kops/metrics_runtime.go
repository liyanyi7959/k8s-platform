...
type MetricsRuntime struct{ service *service.K8sService }

func NewMetricsRuntime(service *service.K8sService) *MetricsRuntime {
	return &MetricsRuntime{service: service}
}
func (r *MetricsRuntime) NodeMetrics(ctx context.Context, clusterID uint64) (any, error) {
	...
	for _, value := range values {
		list = append(list, map[string]any{"name": value.Name, "cpuUsage": value.CPUUsage, "memoryUsage": value.MemoryUsage, "cpuCapacity": value.CPUCapacity, "memoryCapacity": value.MemoryCapacity, "cpuUsed": value.CPUUsed, "memoryUsed": value.MemoryUsed, "source": provider.Name()})
	}
...
func (r *MetricsRuntime) Source(ctx context.Context, clusterID uint64) (any, error) {
	...
	info, err := r.service.GetPrometheusInfo(ctx, clusterID)
	if err != nil {
		return nil, translateKopsRuntimeError(err)
	}
	source, err := r.service.GetClusterMonitorSource(ctx, clusterID)
	if err != nil {
		return nil, translateKopsRuntimeError(err)
	}
	return map[string]any{"monitor_source": source.MonitorSource, "prometheus_url": info.URL, "prometheus_status": info.Status, "prometheus_detected_at": source.PrometheusDetectedAt}, nil
}
func (r *MetricsRuntime) Detect(ctx context.Context, clusterID uint64) (any, error) {
	if r == nil || r.service == nil {
		return nil, kopsapp.ErrConflict
	}
	info, err := r.service.DetectAndUpdatePrometheus(ctx, clusterID)
	if err != nil {
...
func (r *MetricsRuntime) HealthCheck(ctx context.Context, clusterID uint64) (any, error) {
	...
	provider, err := r.service.HealthCheckAndSwitch(ctx, clusterID)
	if err != nil {
		return nil, translateKopsRuntimeError(err)
	}
...
func (r *MetricsRuntime) provider(ctx context.Context, clusterID uint64) (kopsapp.MetricsProvider, error) {
	...
	value, err := r.service.MetricsProvider(ctx, clusterID)
