...
// KubernetesAccess is the narrow cross-context transport contract required by
// Fleet dashboard collection. The Kops Kubernetes adapter satisfies it at the
// composition root without Fleet importing that concrete implementation.
type KubernetesAccess interface {
	GetKubeconfig(context.Context, uint64) (string, error)
...
type DashboardRuntime struct{ k8s KubernetesAccess }

func NewDashboardRuntime(k8s KubernetesAccess) *DashboardRuntime {
