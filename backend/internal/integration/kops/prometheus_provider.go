...
func newPrometheusProvider(manager *MetricsProviderManager) *prometheusProvider {
	return &prometheusProvider{manager: manager}
}
...
func (p *prometheusProvider) client(ctx context.Context, clusterID uint64) (*prometheusClient, error) {
	...
	info, err := p.manager.GetPrometheusInfo(ctx, clusterID)
	if err != nil {
		return nil, err
	}
