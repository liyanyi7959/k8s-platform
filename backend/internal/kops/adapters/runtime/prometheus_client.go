...
// CheckPrometheusHealth provides the narrow health probe required by a
// cross-context provider selector without exposing the transport client.
func CheckPrometheusHealth(ctx context.Context, baseURL string) error {
	return newPrometheusClient(baseURL).health(ctx)
}
