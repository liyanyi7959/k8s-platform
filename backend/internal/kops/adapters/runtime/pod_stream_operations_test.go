...
func TestNormalizePodExecError_KubeletRefused(t *testing.T) {
	...
	if !errors.Is(err, kopsapp.ErrRuntimeNetwork) {
		t.Fatalf("expected ErrRuntimeNetwork, got %v", err)
	}
	message, ok := errorUserMessage(err)
	if !ok {
		t.Fatal("expected user message")
	}
	if message == "" || !containsAll(message, []string{"node4", "10250", "kubelet"}) {
...
func TestNormalizePodExecError_CommandMissing(t *testing.T) {
	...
	if !containsAll(message, []string{"sh", "/bin/sh", "bash"}) {
		t.Fatalf("unexpected message: %q", message)
	}
}
...
func errorUserMessage(err error) (string, bool) {
	var carrier interface{ UserMessage() string }
	if !errors.As(err, &carrier) {
		return "", false
	}
	return carrier.UserMessage(), true
}

type testReader struct{}

func (testReader) Read(p []byte) (int, error) { return 0, nil }
