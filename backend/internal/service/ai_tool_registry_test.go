...
func TestAIToolRegistry_RegisterAndGet(t *testing.T) {
	r := &AIToolRegistry{defs: map[string]AIToolDefinition{}, order: make([]string, 0)}
...
func TestAIToolRegistry_RegisterOverwritesDuplicate(t *testing.T) {
	...
	if count != 1 {
		t.Fatalf("order contains dup %d times, want 1", count)
	}
}
...
func TestHasAllPermissions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := hasAllPermissions(tt.user, tt.req)
			if got != tt.expect {
...
func TestMissingPermissions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := missingPermissions(tt.user, tt.req)
...
func TestAIStringSliceValue_StringSlice(t *testing.T) {
	got := aiStringSliceValue([]string{"a", " b ", "", "c"})
	if len(got) != 3 {
...
func TestAIStringSliceValue_AnySlice(t *testing.T) {
	got := aiStringSliceValue([]any{"x", 123, "y"})
	if len(got) != 3 {
		t.Fatalf("len = %d, want 3", len(got))
	}
	if got[1] != "123" {
...
func TestAIStringSliceValue_DefaultReturnsNil(t *testing.T) {
	if aiStringSliceValue("not a slice") != nil {
		t.Fatal("non-slice should return nil")
	}
	if aiStringSliceValue(nil) != nil {
