...
func NewAIActionService(db *gorm.DB, workloadSvc *WorkloadActionService) *AIActionService {
	return &AIActionService{db: db, workloadSvc: workloadSvc}
}
...
func (s *AIActionService) ConfirmProposal(
	...
	if err := validateAIActionConfirmation(proposal, req); err != nil {
		return ConfirmAIActionProposalResult{}, err
	}
	...
	if proposal.ConfirmLevel == "double" && proposal.ApprovedBy != nil && *proposal.ApprovedBy == userID {
		return ConfirmAIActionProposalResult{}, ErrWithMessage(ErrConflict, "双人确认提案需要由其他成员完成第二次确认")
	}
...
func validateAIActionConfirmation(proposal model.AIActionProposal, req ConfirmAIActionProposalRequest) error {
	...
	confirmationText := strings.TrimSpace(req.ConfirmationText)
	if confirmationText == "" {
		return ErrWithMessage(ErrInvalidParams, "请填写确认短语后再执行")
	}
	requiredText := buildAIActionConfirmationText(proposal.ID)
	if confirmationText != requiredText {
		return ErrWithMessage(ErrInvalidParams, fmt.Sprintf("确认短语不正确，请填写 %s", requiredText))
	}
	return nil
}
