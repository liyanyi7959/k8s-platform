...
const aiChatMaxFileTextRunes = aiapp.ChatMaxFileTextRunes
...
type AIGatewayResponse struct {
	...
	SuggestedActions []aidomain.AISuggestedAction
...
func (s *AIGatewayService) Invoke(ctx context.Context, req AIGatewayRequest) (AIGatewayResponse, error) {
	...
	default:
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, "当前提供商类型暂不支持在线调用")
	}
}
...
func (s *AIGatewayService) ResolveInvocationTarget(ctx context.Context, req AIGatewayRequest) (aidomain.AIProvider, aidomain.AIModel, string, error) {
	return s.resolveInvocationTarget(ctx, req)
}

func (s *AIGatewayService) resolveInvocationTarget(ctx context.Context, req AIGatewayRequest) (aidomain.AIProvider, aidomain.AIModel, string, error) {
	var aiModel aidomain.AIModel
	...
	if err := q.Order("p.priority ASC, m.id DESC").First(&aiModel).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return aidomain.AIProvider{}, aidomain.AIModel{}, "", aiapp.ErrorWithMessage(aiapp.ErrNotFound, "未找到可用的 AI 模型")
		}
		return aidomain.AIProvider{}, aidomain.AIModel{}, "", err
	}
	...
	} else if !errors.Is(providerErr, aiapp.ErrNotFound) {
		return aidomain.AIProvider{}, aidomain.AIModel{}, "", providerErr
	...
	if err := s.db.WithContext(ctx).
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return aidomain.AIProvider{}, aidomain.AIModel{}, "", aiapp.ErrorWithMessage(aiapp.ErrNotFound, "未找到可用的 AI 提供商")
		}
		return aidomain.AIProvider{}, aidomain.AIModel{}, "", err
	}
	...
	if provider.APIKeyEnc != nil && strings.TrimSpace(*provider.APIKeyEnc) != "" {
		plain, err := aiapp.DecryptText(s.encryptionKey, *provider.APIKeyEnc)
		if err != nil {
			return aidomain.AIProvider{}, aidomain.AIModel{}, "", err
		}
...
func (s *AIGatewayService) findRoutePreferredModel(ctx context.Context, req AIGatewayRequest) *uint64 {
	...
	switch aidomain.NormalizeRoutingStrategy(settings.RoutingStrategy) {
...
func (s *AIGatewayService) loadRouteSettings(ctx context.Context) (*aidomain.AIRouteSetting, error) {
...
func (s *AIGatewayService) findFallbackModel(ctx context.Context, req AIGatewayRequest, providerID uint64) (aidomain.AIModel, error) {
	var row aidomain.AIModel
	if err := s.baseEnabledModelQuery(ctx, req).
		Where("m.provider_id = ?", providerID).
		Order("m.id DESC").
		First(&row).Error; err != nil {
		return aidomain.AIModel{}, err
	}
	return row, nil
}

func (s *AIGatewayService) loadProviderWithKey(ctx context.Context, providerID uint64) (aidomain.AIProvider, string, error) {
	var provider aidomain.AIProvider
	if err := s.db.WithContext(ctx).
		Where("deleted_at IS NULL AND enabled = 1 AND id = ?", providerID).
		First(&provider).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return aidomain.AIProvider{}, "", aiapp.ErrNotFound
		}
		return aidomain.AIProvider{}, "", err
	}
...
func (s *AIGatewayService) invokeMock(provider aidomain.AIProvider, aiModel aidomain.AIModel, req AIGatewayRequest) AIGatewayResponse {
	lastUser := ""
...
func (s *AIGatewayService) invokeOpenAICompatible(
	ctx context.Context,
	provider aidomain.AIProvider,
	aiModel aidomain.AIModel,
	apiKey string,
	req AIGatewayRequest,
) (AIGatewayResponse, error) {
	if apiKey == "" {
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, "当前 AI 提供商未配置 API Key")
	}
	...
	if err != nil {
		...
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrProviderUnavailable, "AI 提供商连接失败")
	}
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, fmt.Sprintf("AI 提供商返回异常状态: %d, 响应: %s", httpResp.StatusCode, errBody))
	}
	...
	if err := json.Unmarshal(respBody, &parsed); err != nil {
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, "AI 提供商响应格式无法解析")
	}
	if len(parsed.Choices) == 0 {
		return AIGatewayResponse{}, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, "AI 提供商未返回有效回答")
	}
...
func (s *AIGatewayService) InvokeStream(ctx context.Context, req AIGatewayRequest) (io.ReadCloser, AIGatewayStreamResult, error) {
	...
	if apiKey == "" {
		return nil, result, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, "当前 AI 提供商未配置 API Key")
	}
	...
	if err != nil {
		...
		return nil, result, aiapp.ErrorWithMessage(aiapp.ErrProviderUnavailable, "AI 提供商连接失败")
	}
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		errBody := ""
		if b, readErr := io.ReadAll(io.LimitReader(httpResp.Body, 2048)); readErr == nil {
		...
		_ = httpResp.Body.Close()
		return nil, result, aiapp.ErrorWithMessage(aiapp.ErrInvalidParams, fmt.Sprintf("AI 提供商返回异常状态: %d, 响应: %s", httpResp.StatusCode, errBody))
	}
...
func BuildScopeNote(namespace, kind, name string) string {
	return buildAIGatewayScopeNote(namespace, kind, name)
}
