...
const (
	UploadKindImage      = aiUploadKindImage
	UploadKindText       = aiUploadKindText
	ChatMaxFileTextRunes = aiChatMaxFileTextRunes
)
...
func (s *AIFileService) SaveChatUploads(
	...
	rows := make([]aidomain.AIUploadedFile, 0, len(uploads))
	...
	for _, upload := range uploads {
		...
		rows = append(rows, aidomain.AIUploadedFile{
...
func buildAIMessageAttachmentItem(row aidomain.AIUploadedFile) AIMessageAttachmentItem {
