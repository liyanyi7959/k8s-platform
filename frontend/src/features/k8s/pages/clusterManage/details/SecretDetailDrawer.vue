        ...
        <div class="k8s-tab-pane">
          <div class="k8s-pane-toolbar">
            <el-space :size="8" wrap>
              ...
              <el-switch v-model="decode" inline-prompt active-text="解码" inactive-text="base64" />
              <el-tag v-if="isMaskedValue" size="small" type="warning">默认脱敏</el-tag>
              <el-tag v-if="decode" size="small" type="warning">敏感信息</el-tag>
          ...
          <el-card shadow="never" class="k8s-section-card k8s-accent-card">
            <template #header>
              <div class="k8s-section-title-row">
                <div class="k8s-section-title">Pods（引用该 Secret）</div>
...
const detailTitle = computed(() => (detailName.value ? `Secret 详情：${detailName.value}` : 'Secret 详情'))
const createdAtText = computed(() => formatTs(row.value?.metadata?.creationTimestamp))
const typeText = computed(() => String(row.value?.type ?? ''))
...
const isMaskedValue = computed(() => activeBase64.value === '***')
const activeDecoded = computed(() => { try { return decodeBase64Utf8(activeBase64.value) } catch { return '' } })
const activePretty = computed(() => tryPrettyJson(decode.value ? activeDecoded.value : activeBase64.value))
const activeFormat = computed(() => (activePretty.value.ok ? 'json' : 'text'))
const activeViewText = computed(() => {
  if (isMaskedValue.value) {
    return '已默认脱敏，请使用列表中的“查看内容”按钮申请明文查看。'
  }
  return normalizeMultilineText(activePretty.value.text)
})
...
function downloadActiveKey() {
  ...
  downloadBlob(`secret_${sanitizeFileName(`${detailNamespace.value}_${detailName.value}`)}_${sanitizeFileName(String(activeKey.value))}_${ts}.txt`, new Blob([text], { type: 'text/plain;charset=utf-8' }))
}
...
async function loadRelated() {
  ...
  try {
    const data = await k8sApi.getSecretRelated(props.clusterId, ns, name)
    ...
    if (status === 404 || err.code === 4040 || err.code === 4000) {
      try {
        ...
        const pods = (Array.isArray(data.list) ? data.list : []).filter((p: any) => podUsesSecret(p, name)).map((p: any) => toRelatedPodVmFromPod(p)).sort((a: any, b: any) => `${a.namespace}/${a.name}`.localeCompare(`${b.namespace}/${b.name}`))
        ...
        const cFromWl: ControllerRow[] = (Array.isArray(w.list) ? w.list : []).filter((it: any) => workloadUsesSecret(it, name)).map((it: any) => ({ kind: String(it?.kind ?? '').trim(), namespace: String(it?.metadata?.namespace ?? ns).trim(), name: String(it?.metadata?.name ?? '').trim() })).filter((c: ControllerRow) => c.kind && c.namespace && c.name)
        const ing = await k8sApi.listIngresses(props.clusterId, { namespace: ns })
        const cFromIng: ControllerRow[] = (Array.isArray(ing.list) ? ing.list : []).filter((it: any) => ingressUsesSecret(it, name)).map((it: any) => ({ kind: 'Ingress', namespace: String(it?.metadata?.namespace ?? ns).trim(), name: String(it?.metadata?.name ?? '').trim() })).filter((c: ControllerRow) => c.namespace && c.name)
        relatedControllers.value = mergeControllers(cFromPods, cFromWl, cFromIng)
...
async function loadEvents() {
  ...
  try {
    ...
    const filtered = list.filter((ev) => {
      ...
      if (String(inv?.kind ?? '') && String(inv?.kind ?? '') !== 'Secret') return false
...
watch(() => visible.value, (v) => {
  if (v) return
  tab.value = 'overview'; row.value = null; activeKey.value = ''; decode.value = false; wrap.value = true; lineNumbers.value = false
