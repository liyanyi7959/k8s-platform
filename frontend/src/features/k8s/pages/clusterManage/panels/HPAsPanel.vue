<template>
  ...
  >
    ...
    <template #cell-target="{ row }">
      <span class="k8s-num">
        {{ `${String(row?.spec?.scaleTargetRef?.kind ?? '-')}/${String(row?.spec?.scaleTargetRef?.name ?? '-')}` }}
      </span>
    </template>
    <template #cell-replicas="{ row }">
      <span class="k8s-num">{{ `${Number(row?.status?.currentReplicas ?? 0)}/${Number(row?.status?.desiredReplicas ?? 0)}` }}</span>
    </template>
...
const columns: EnhancedColumn[] = [
  ...
  { key: 'target', label: 'Target', minWidth: 200, defaultVisible: true },
  { key: 'min', label: 'Min', prop: 'spec.minReplicas', width: 90, sortable: 'custom', align: 'center', headerAlign: 'center', defaultVisible: true },
