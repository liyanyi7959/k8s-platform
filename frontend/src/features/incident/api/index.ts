...
export function listAlertEvents(clusterId?: number, signal?: AbortSignal): Promise<AlertEvent[]> {
  ...
  return listIncidents({ page: 1, pageSize: 100 }, signal).then(({ items }) => items
    .filter((incident) => !clusterId || incident.clusterId === clusterId)
    .map((incident) => ({
      id: incident.id,
      ruleId: 0,
      ruleName: incident.alertName,
...
export function listIncidents(params?: { page?: number; pageSize?: number; status?: string }, signal?: AbortSignal) {
  return request('/api/v2/incidents', {
...
export function getIncident(id: number): Promise<{ incident: MonitorIncident; timeline: IncidentTimelineItem[] }> {
  return request(`/api/v2/incidents/${id}`).then((raw) => camelize(raw))
}

const incidentCommandResources = {
  acknowledge: 'acknowledgements',
  diagnose: 'diagnosis-runs',
  await_approval: 'approval-requests',
  execute: 'execution-starts',
  verify: 'verification-runs',
  resolve: 'resolution-attempts',
} as const

export type IncidentAction = keyof typeof incidentCommandResources

function postIncidentCommand(id: number, action: IncidentAction, expectedVersion: number, note = ''): Promise<MonitorIncident> {
  return request(`/api/v2/incidents/${id}/${incidentCommandResources[action]}`, {
    method: 'POST',
    data: { expected_version: expectedVersion, note },
  }).then((raw) => camelize(raw))
}

export const acknowledgeIncident = (id: number, version: number, note = '') => postIncidentCommand(id, 'acknowledge', version, note)
export const startIncidentDiagnosis = (id: number, version: number, note = '') => postIncidentCommand(id, 'diagnose', version, note)
export const requestIncidentApproval = (id: number, version: number, note = '') => postIncidentCommand(id, 'await_approval', version, note)
export const startIncidentExecution = (id: number, version: number, note = '') => postIncidentCommand(id, 'execute', version, note)
export const startIncidentVerification = (id: number, version: number, note = '') => postIncidentCommand(id, 'verify', version, note)
export const resolveIncident = (id: number, version: number, note = '') => postIncidentCommand(id, 'resolve', version, note)
...
export function listEvents(
  ...
  return request(`/api/v2/clusters/${params.clusterId}/events`, {
    ...
    const items = camelize<K8sEventItem[]>(raw?.list || raw?.items || [])
    return { items, total: items.length, page: params.page || 1, pageSize: params.pageSize || 20 }
  })
}
