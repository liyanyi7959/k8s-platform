...
import type { CreateServerRequest, DeployServer } from '@/features/provisioning/types'
...
export default function ServersPage() {
  ...
  const testMutation = useMutation({
    mutationFn: async (id: number) => {
      setTestingId(id)
      return probeServerConnection(id)
    },
  ...
  const runBatchInspection = async () => {
    ...
    const results = await Promise.allSettled(targets.map((server) => probeServerConnection(server.id)))
