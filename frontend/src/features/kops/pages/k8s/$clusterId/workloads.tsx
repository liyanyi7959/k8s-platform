...
export default function WorkloadsPage({ fixedKind }: WorkloadsPageProps) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: ({ signal }) => getWorkloads(clusterId, namespace || undefined, signal, workloadKind),
    enabled: !!clusterId,
  ...
  const { data: detailHistoryData, isLoading: detailHistoryLoading } = useQuery({
    ...
    queryFn: ({ signal }) => getDeploymentHistory(clusterId, detailDrawer.record?.namespace || '', detailDrawer.record?.name || '', signal),
    enabled: !!clusterId && detailDrawer.open && !!detailDrawer.record?.namespace && !!detailDrawer.record?.name,
  })
  ...
  const { data: detailYamlData, isLoading: detailYamlLoading } = useQuery({
    ...
    queryFn: async ({ signal }) => {
      const res = await getDeploymentYaml(clusterId, detailDrawer.record?.namespace || '', detailDrawer.record?.name || '', signal, workloadKind)
      return res.yaml || ''
    },
    enabled: !!clusterId && detailDrawer.open && !!detailDrawer.record?.namespace && !!detailDrawer.record?.name,
  })
  ...
  const createMutation = useMutation({
    mutationFn: (values: any) =>
      createWorkload(clusterId, values.namespace || 'default', values, workloadKind),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ name, namespace: recordNamespace }: { name: string; namespace: string }) =>
      deleteWorkload(clusterId, recordNamespace, name, workloadKind),
    onSuccess: () => {
  ...
  const scaleMutation = useMutation({
    mutationFn: ({ name, namespace: recordNamespace, replicas }: { name: string; namespace: string; replicas: number }) =>
      scaleWorkload(clusterId, recordNamespace, name, replicas, workloadKind),
    onSuccess: () => {
  ...
  const restartMutation = useMutation({
    mutationFn: ({ name, namespace: recordNamespace }: { name: string; namespace: string }) =>
      restartWorkload(clusterId, recordNamespace, name, workloadKind),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ name, namespace: recordNamespace, data }: { name: string; namespace: string; data: any }) =>
      updateWorkload(clusterId, recordNamespace, name, { ...data, kind: workloadKind }),
    onSuccess: () => {
                ...
                ? [
                    {
                      ...
                      children: (
                        <Table
                          ...
                          columns={[
                            { title: '版本', dataIndex: 'revision', width: 70, render: (v, _: any, i: number) => <Tag>v{v || i + 1}</Tag> },
                            { title: '变更原因', dataIndex: 'changeCause', ellipsis: true, render: (v) => v || '-' },
                            { title: '镜像', dataIndex: 'images', ellipsis: true, render: (_, r: any) => r.images?.join(', ') || r.image || '-' },
