...
export function triggerCronJob(clusterId: number, namespace: string, name: string): Promise<any> {
  return request(`/api/v1/clusters/${clusterId}/cronjobs/${namespace}/${name}/execution-requests`, { method: 'POST' })
}

/** CronJob 暂停/恢复 */
export function suspendCronJob(clusterId: number, namespace: string, name: string, suspend: boolean): Promise<any> {
  return request(`/api/v1/clusters/${clusterId}/cronjobs/${namespace}/${name}/suspension-state`, { method: 'PATCH', data: { suspend } })
}
...
export function updateHPA(clusterId: number, namespace: string, name: string, data: any): Promise<any> {
  return request(`/api/v1/clusters/${clusterId}/hpas/${namespace}/${name}`, { method: 'PATCH', data: { namespace, yaml: buildHPAYaml(name, namespace, data) } })
}
