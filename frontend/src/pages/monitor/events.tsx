...
const nextAction: Partial<Record<IncidentStatus, { action: string; label: string }>> = {
...
const nextStatusByAction: Record<string, IncidentStatus> = {
  acknowledge: 'acknowledged', diagnose: 'diagnosing', await_approval: 'awaiting_approval',
  execute: 'executing', verify: 'verifying', resolve: 'resolved',
}
...
const EventsPage: React.FC = () => {
  ...
  const transition = useMutation({
    mutationFn: ({ id, action }: { id: number; action: string }) => transitionIncident(id, action),
    onSuccess: (_, variables) => {
      message.success('事件状态已更新')
      setSelected((current) => current && current.id === variables.id
        ? { ...current, status: nextStatusByAction[variables.action] || current.status }
        : current)
    ...
    onError: (error) => message.error(error instanceof Error ? error.message : '事件状态更新失败'),
  })
  ...
  const openAI = () => {
    if (!selected) return
    transition.mutate({ id: selected.id, action: 'diagnose' }, {
          ...
          <Space direction="vertical" size={20} style={{ width: '100%' }}>
            ...
            <div className="app-incident-next-action">
              ...
              {selected.status === 'acknowledged' ? (
                ...
                <Button type="primary" danger={selectedNextAction.action === 'resolve'} onClick={() => transition.mutate({ id: selected.id, action: selectedNextAction.action })} loading={transition.isPending}>
