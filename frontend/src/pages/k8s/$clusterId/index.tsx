...
const K8sDashboardPage: React.FC = () => {
    ...
    return (
      <AppPage>
        <Empty description="暂无集群概览数据">
          <Button type="primary" icon={<ReloadOutlined />} onClick={() => refetch()}>重新加载</Button>
        </Empty>
      </AppPage>
    )
  }
  ...
  const nodeReadyPercent = overview.charts.node_ready.total > 0
  ...
  return (
    <AppPage>
        ...
        extra={
          <Space>
            {overview.meta?.updated_at && (
              <Tooltip title={`来源: ${overview.meta.source || 'k8s-api'} | 更新: ${dayjs(overview.meta.updated_at).format('HH:mm:ss')}`}>
                <Text type="secondary" style={{ fontSize: 11 }}>
                  <ClockCircleOutlined /> {timeAgo(overview.meta.updated_at)}
                </Text>
              </Tooltip>
            )}
            <Switch checkedChildren="自动" unCheckedChildren="手动" checked={autoRefresh} onChange={setAutoRefresh} size="small" />
            {autoRefresh && (
              <Select size="small" value={refreshInterval} onChange={setRefreshInterval} style={{ width: 65 }}
          ...
          {[
            ...
            <Col xs={12} sm={6} key={kpi.label}>
              <Card hoverable size="small" style={{ borderLeft: `3px solid ${kpi.color}`, cursor: kpi.path ? 'pointer' : 'default' }}
                ...
                <Space>
                  <div style={{ width: 38, height: 38, borderRadius: 8, background: `${kpi.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: kpi.color, fontSize: 18 }}>{kpi.icon}</div>
                  <div>
                    <Text type="secondary" style={{ fontSize: 11 }}>{kpi.label}</Text>
                    <div style={{ fontSize: 22, fontWeight: 900, lineHeight: 1.1 }}>{kpi.value}</div>
                    <Text type="secondary" style={{ fontSize: 10 }}>{kpi.sub}</Text>
                  </div>
                </Space>
              </Card>
            </Col>
      ...
      <Row gutter={12} style={{ marginBottom: 12 }}>
        <Col xs={24} md={10}>
          <Card title={<Space><DashboardOutlined style={{ color: COLOR.primary }} /> 集群健康度</Space>} size="small" style={{ height: '100%' }}>
            <Row align="middle" gutter={16}>
              <Col span={10} style={{ textAlign: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block' }}>
                  <Progress type="dashboard" percent={healthScore} size={140}
                    strokeColor={getHealthColor(getHealthTier(healthScore))}
                    format={() => <div><div style={{ fontSize: 28, fontWeight: 900 }}>{healthScore}</div><div style={{ fontSize: 10 }}>健康分</div></div>} />
                </div>
              </Col>
              <Col span={14}>
                {healthBreakdown.map((item) => (
                  <div key={item.key} style={{ marginBottom: 10 }}>
                    <Row justify="space-between" style={{ marginBottom: 2 }}>
                      ...
                      <Text style={{ color: getHealthColor(item.tier), fontSize: 12, fontWeight: 600 }}>{item.score}分</Text>
                    </Row>
                    <Progress percent={item.score} size="small" strokeColor={getHealthColor(item.tier)} showInfo={false} />
                    <Text type="secondary" style={{ fontSize: 10 }}>{item.hint}</Text>
                  </div>
                ))}
              </Col>
            </Row>
        ...
        <Col xs={24} md={7}>
          <Card title={<Space><AppstoreOutlined style={{ color: COLOR.purple }} /> Pod 状态分布</Space>} size="small" style={{ height: '100%' }}>
            {podPhaseData.length === 0 ? (
              ...
              <>
                <Suspense fallback={<Spin style={{ display: 'block', margin: '50px auto' }} />}>
                  <LazyPie height={140} data={podPhaseData} angleField="value" colorField="type"
                    color={[COLOR.healthy, COLOR.warning, COLOR.critical, COLOR.idle]}
                    innerRadius={0.55} legend={false}
                ...
                <Row justify="center" gutter={16}>
                  {[
                    ...
                    <Space key={p.label} size={4}>
                      ...
                      <Text style={{ fontSize: 11 }}>{p.label} {p.value}</Text>
                    </Space>
                  ))}
                </Row>
              </>
        ...
        <Col xs={24} md={7}>
          <Card title={<Space><NodeIndexOutlined style={{ color: COLOR.primary }} /> 节点就绪</Space>} size="small" style={{ height: '100%' }}>
            <div style={{ textAlign: 'center' }}>
              <Progress type="circle" percent={nodeReadyPercent} size={140}
                strokeColor={nodeReadyPercent === 100 ? COLOR.healthy : nodeReadyPercent >= 50 ? COLOR.warning : COLOR.critical}
                format={() => <div><div style={{ fontSize: 28, fontWeight: 900 }}>{overview.charts.node_ready.ready}/{overview.charts.node_ready.total}</div><div style={{ fontSize: 10 }}>Ready 节点</div></div>} />
            </div>
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={12} style={{ marginBottom: 12 }}>
        <Col xs={24} md={8}>
          <Card title={<Space><CloseCircleFilled style={{ color: COLOR.critical }} /> 异常 Pod <Badge count={failedPods.length} style={{ backgroundColor: COLOR.critical }} /></Space>} size="small" style={{ height: '100%' }}
            bodyStyle={{ maxHeight: 200, overflow: 'auto' }}>
            {failedPods.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="无异常" /> : (
              <List size="small" dataSource={failedPods} renderItem={(item) => (
                <List.Item style={{ padding: '4px 0', cursor: 'pointer' }} onClick={() => history.push(`/k8s/${clusterId}/pods`)}>
                  <Space direction="vertical" size={0} style={{ width: '100%' }}>
                    <Row justify="space-between">
                      <EllipsisText text={item.name} />
                      <Tag color="error" style={{ fontSize: 10 }}>{item.reason}</Tag>
                    </Row>
                    <Text type="secondary" style={{ fontSize: 10 }}>{item.namespace}</Text>
                  </Space>
                </List.Item>
              )} />
            )}
        ...
        <Col xs={24} md={8}>
          <Card title={<Space><WarningOutlined style={{ color: COLOR.warning }} /> 未调度 Pod <Badge count={unscheduledPods.length} style={{ backgroundColor: COLOR.warning }} /></Space>} size="small" style={{ height: '100%' }}
            bodyStyle={{ maxHeight: 200, overflow: 'auto' }}>
            {unscheduledPods.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="无未调度" /> : (
              <List size="small" dataSource={unscheduledPods} renderItem={(item) => (
                <List.Item style={{ padding: '4px 0', cursor: 'pointer' }} onClick={() => history.push(`/k8s/${clusterId}/pods`)}>
                  <Space direction="vertical" size={0} style={{ width: '100%' }}>
                    <EllipsisText text={item.name} />
                    <Text type="secondary" style={{ fontSize: 10 }}>{item.namespace}</Text>
        ...
        <Col xs={24} md={8}>
          <Card title={<Space><LockOutlined style={{ color: certRisks.length > 0 ? COLOR.warning : COLOR.healthy }} /> 证书过期风险 <Badge count={certRisks.length} style={{ backgroundColor: COLOR.warning }} /></Space>} size="small" style={{ height: '100%' }}
            bodyStyle={{ maxHeight: 200, overflow: 'auto' }}>
            {certRisks.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="无证书风险" /> : (
              <List size="small" dataSource={certRisks} renderItem={(cert) => (
                <List.Item style={{ padding: '6px 0' }}>
                  <div style={{ width: '100%' }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 10 }}>{cert.component} · {cert.purpose}</Text>
                  </div>
                </List.Item>
              )} />
            )}
      ...
      <Card
        title={
          <Space>
            <WarningOutlined style={{ color: warningEvents.length > 0 ? COLOR.critical : COLOR.idle }} />
            <span>近期告警事件</span>
      ...
      <Row gutter={12}>
        <Col xs={24} lg={10}>
          <Card title={<Space><AppstoreOutlined style={{ color: COLOR.purple }} /> Namespace Pod Top 10</Space>} size="small">
            {overview.charts.namespace_pods_top.length === 0 ? (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无数据" style={{ marginTop: 60 }} />
            ) : (
              <Suspense fallback={<Spin style={{ display: 'block', margin: '80px auto' }} />}>
                <LazyColumn data={overview.charts.namespace_pods_top} xField="namespace" yField="pods" height={220}
                  color={COLOR.purple} columnStyle={{ radius: [4, 4, 0, 0] }}
                  xAxis={{ label: { autoRotate: true, style: { fontSize: 10 } } }}
        ...
        <Col xs={24} lg={14}>
          <Card title={<Space><CloudServerOutlined style={{ color: COLOR.primary }} /> Top 5 工作负载快照</Space>} size="small"
