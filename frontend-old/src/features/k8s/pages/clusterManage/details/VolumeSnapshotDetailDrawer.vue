  ...
  >
    <template #summary>
      <div class="k8s-summary-grid">
        ...
        <div class="k8s-kv k8s-kv--muted"><div class="k8s-k">Ready:</div><div class="k8s-v">{{ readyText }}</div></div>
        <div class="k8s-kv k8s-kv--muted"><div class="k8s-k">Source PVC:</div><div class="k8s-v">{{ sourcePvcText }}</div></div>
      </div>
    </template>
        ...
        <div class="k8s-tab-pane">
          <el-card shadow="never" class="k8s-section-card k8s-accent-card">
            ...
            <el-descriptions :column="3" size="small" border class="k8s-detail-desc">
              ...
              <el-descriptions-item label="readyToUse">{{ readyText }}</el-descriptions-item>
              <el-descriptions-item label="source PVC">{{ sourcePvcText }}</el-descriptions-item>
              <el-descriptions-item label="snapshotClass">{{ snapshotClassText }}</el-descriptions-item>
              <el-descriptions-item label="bound content">{{ boundContentText }}</el-descriptions-item>
              <el-descriptions-item label="restore size">{{ restoreSizeText }}</el-descriptions-item>
              <el-descriptions-item label="error">{{ errorText }}</el-descriptions-item>
            </el-descriptions>
          </el-card>
          <el-card shadow="never" class="k8s-section-card k8s-accent-card">
...
const detailTitle = computed(() => (detailName.value ? `VolumeSnapshot 详情：${detailName.value}` : 'VolumeSnapshot 详情'))
const createdAtText = computed(() => formatTs(row.value?.metadata?.creationTimestamp))
const readyText = computed(() => boolText(row.value?.status?.readyToUse))
const sourcePvcText = computed(() => String(row.value?.spec?.source?.persistentVolumeClaimName ?? '-').trim() || '-')
const snapshotClassText = computed(() => String(row.value?.spec?.volumeSnapshotClassName ?? '-').trim() || '-')
const boundContentText = computed(() => String(row.value?.status?.boundVolumeSnapshotContentName ?? '-').trim() || '-')
const restoreSizeText = computed(() => String(row.value?.status?.restoreSize ?? '-').trim() || '-')
const errorText = computed(() => String(row.value?.status?.error?.message ?? row.value?.status?.error?.reason ?? '-').trim() || '-')
const specViewText = computed(() => JSON.stringify(row.value?.spec ?? {}, null, 2))
const statusViewText = computed(() => JSON.stringify(row.value?.status ?? {}, null, 2))
...
async function loadEvents() {
  ...
  try {
    const data = await k8sApi.listEvents(props.clusterId, {
      namespace,
      involved_object_kind: 'VolumeSnapshot',
...
async function refreshDetail() {
  ...
  try {
    ...
    const data = await k8sApi.listVolumeSnapshots(props.clusterId, { namespace })
