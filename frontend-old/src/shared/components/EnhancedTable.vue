...
function computeAutoFitHeight() {
  ...
  const availableH = container.clientHeight - topH - botH

  // 按数据行数估算内容实际需要的高度
  const rowCount = props.data.length
  const rowHeight = tableSize.value === 'small' ? 36 : tableSize.value === 'large' ? 56 : 48
  const contentH = rowCount * rowHeight + 40

  if (availableH > 0) {
    // 取可用高度与内容需要的较小值，数据少时不强制表格填满容器
    const target = Math.min(availableH, contentH)
    autoFitHeight.value = Math.max(120, target)
  }
}
...
/* ── 居中列表头内容居中（兜底 header-align prop） ── */
.enhanced-table :deep(th.el-table__cell .cell) {
  text-align: inherit;
}
...
/* 消除 el-table 底部左侧 border-patch 占位 */
.enhanced-table :deep(.el-table__border-left-patch) {
  display: none !important;
}
